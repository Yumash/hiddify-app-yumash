...
// MakeRule creates a RawDefaultRule from the Rule struct
// The caller is responsible for wrapping this in DefaultRule with RuleAction
func (r *Rule) MakeRule() option.RawDefaultRule {
	rule := option.RawDefaultRule{}
	if len(r.Domains) > 0 {
...
func (r *Rule) MakeDNSRule() option.RawDefaultDNSRule {
...
func makeDomainRule(options option.RawDefaultRule, list []string) option.RawDefaultRule {
...
func makeIpRule(options option.RawDefaultRule, list []string) option.RawDefaultRule {
	for _, item := range list {
...
func makePortRule(options option.RawDefaultRule, list []string) option.RawDefaultRule {
	for _, item := range list {
