...
func (s *CoreService) MainOutboundsInfo(req *pb.Empty, stream grpc.ServerStreamingServer[pb.OutboundGroupList]) error {
	// In sing-box 1.12+, CommandGroupInfoOnly was removed, using CommandGroup instead
...
// ClearRuleSetCache deletes the cache.db file to force re-download of rule-sets
// Should be called when service is stopped
func ClearRuleSetCache() error {
	cacheFile := filepath.Join(sWorkingPath, "cache.db")
...
func (s *CoreService) ClearRuleSetCache(ctx context.Context, in *pb.Empty) (*pb.Response, error) {
	err := ClearRuleSetCache()
	if err != nil {
		return &pb.Response{
			ResponseCode: pb.ResponseCode_FAILED,
			Message:      err.Error(),
	...
	return &pb.Response{
		ResponseCode: pb.ResponseCode_OK,
		Message:      "Rule-set cache cleared",
	}, nil
}
