...
func getRandomAvailablePort() uint16 {
...
func RunInstance(hiddifySettings *config.HiddifyOptions, singconfig *option.Options) (*HiddifyService, error) {
	...
	hiddifySettings.InboundOptions.MixedPort = getRandomAvailablePort()
	...
	// Build JSON config instead of options for libbox.NewService
	finalConfigJson, err := config.BuildConfigJson(*hiddifySettings, *singconfig)
	if err != nil {
		return nil, err
	}
