...
// dataDir is the directory for LevelDB databases, set by SetDataDir
var dataDir = "./data"

// SetDataDir sets the directory for LevelDB databases
func SetDataDir(dir string) {
	...
	if err := os.MkdirAll(dataDir, 0755); err != nil {
		...
		log.Printf("LevelDB data directory set to: %s", dataDir)
	}
}
...
func getDB(name string, readOnly bool) tmdb.DB {
	...
	for i := 0; i < retryAttempts; i++ {
		db, err := tmdb.NewGoLevelDBWithOpts(name, dataDir, &opt.Options{ReadOnly: readOnly})
		if err == nil {
			return db
		}
		if i == 0 {
			log.Printf("Failed to open database %s in %s: %v (will retry %d times)", name, dataDir, err, retryAttempts)
		}
		time.Sleep(retryDelay)
	}
	log.Printf("Giving up opening database %s in %s after %d attempts", name, dataDir, retryAttempts)
	return nil
}
