...
func writeGroups(writer io.Writer, svc ServiceInterface) error {
	ctx := svc.GetContext()
	instance := svc.GetInstance()
	if ctx == nil || instance == nil {
		return varbin.Write(writer, binary.BigEndian, []OutboundGroup{})
	}
	...
	for _, iGroup := range iGroups {
		...
		for _, itemTag := range iGroup.All() {
			itemOutbound, isLoaded := instance.Outbound().Outbound(itemTag)
			if !isLoaded {
				continue
			}
