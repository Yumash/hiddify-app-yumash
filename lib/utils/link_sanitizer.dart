...
class LinkSanitizer with InfraLogger {
  ...
  static String sanitizeLink(String link) {
    ...
    if (sanitized.contains('fake_ip_for_sub_link')) {
      loggy.debug('Sanitizing fake_ip_for_sub_link -> 1.1.1.1');
    ...
    if (invalidHostnamePattern.hasMatch(sanitized)) {
      loggy.debug('Found invalid hostname pattern, replacing with cloudflare-dns.com');
    ...
    try {
      ...
      if (uri.host.isNotEmpty) {
        ...
        if (uri.host != normalizedHost) {
          loggy.debug('Normalizing mixed case domain: ${uri.host} -> $normalizedHost');
  ...
  static List<String> sanitizeLinks(List<String> links) {
    ...
    if (fixedCount > 0) {
      loggy.info('Sanitized $fixedCount problematic links out of ${links.length}');
    }
